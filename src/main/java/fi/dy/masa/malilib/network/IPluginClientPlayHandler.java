 ...
 * @param <T> (Payload)
 */
public interface IPluginClientPlayHandler<T extends CustomPayload> extends IPayloadHandler<T>
{
    int FROM_SERVER = 1;
    int TO_SERVER = 2;
    int BOTH_SERVER = 3;
    ...
    /**
     * Register your Payload with NeoForge network.
     * See the NeoForge's network Java Docs under PayloadTypeRegistry -> register()
    ...
    default void registerPlayPayload(@Nonnull CustomPayload.Id<T> id, @Nonnull PacketCodec<? super RegistryByteBuf,T> codec, int direction)
    {
        if (this.isPlayRegistered(this.getPayloadChannel()) == false)
        {
            try
            {
                switch (direction)
                {
                    case TO_SERVER, FROM_CLIENT -> NeoNetwork.playToServer(id, codec, this);
                    case FROM_SERVER, TO_CLIENT -> NeoNetwork.playToClient(id, codec, this);
                    default -> NeoNetwork.playBidirectional(id, codec, this);
                }
            }
     ...
     * See the NeoForge's network Java Docs under PayloadRegistrar.playToClient()
     * for more information on how to do this.
     * -
     * @param id (Your Payload Id<T>)
     * @param codec (Your Payload's CODEC)
     ...
     * @return (True / False)
     */
    default boolean registerPlayReceiver(@Nonnull CustomPayload.Id<T> id, @Nonnull PacketCodec<? super RegistryByteBuf,T> codec, @Nullable IPayloadHandler<T> receiver)
    {
        if (this.isPlayRegistered(this.getPayloadChannel()))
        {
            try
            {
                NeoNetwork.playToClient(id, codec, Objects.requireNonNullElse(receiver, this::receivePlayPayload));
                return true;
            }
            catch (IllegalArgumentException e)
            {
...
//    }
     ...
     * @param ctx (NeoForge's network Context)
     */
    ...
    /**
     * Sends the Payload to the server using the NeoForge's network interface.
     * -
    ...
    default boolean sendPlayPayload(@Nonnull T payload)
    {
        ...
        {
            MaLiLib.logger.warn("sendPlayPayload: [NeoForge-Network] error sending payload for channel: {}, check if channel is registered", payload.getId().id().toString());
        }
